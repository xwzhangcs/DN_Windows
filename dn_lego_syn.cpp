...
int main(int argc, const char* argv[]) {
	...
	if(false)
	{
		...
		std::vector<std::string> images = get_all_files_names_within_folder(path + "/enc_net_convt");
		std::cout << "images size is " << images.size() << std::endl;
		for (int index = 0; index < images.size(); index++) {
			std::string img_name = path + "/enc_net_convt/" + images[index];
			cv::Mat src_img = cv::imread(img_name, CV_LOAD_IMAGE_UNCHANGED);
			cv::Mat real_img = cv::imread(path + "/A/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
			cv::Mat scale_img;
			cv::resize(src_img, scale_img, real_img.size());
			cv::Mat seg_out = scale_img.clone();
			// correct the color
			for (int i = 0; i < scale_img.size().height; i++) {
				for (int j = 0; j < scale_img.size().width; j++) {
					//noise
					if (scale_img.at<cv::Vec3b>(i, j)[2] > 128) {
						seg_out.at<cv::Vec3b>(i, j)[0] = 0;
						seg_out.at<cv::Vec3b>(i, j)[1] = 0;
						seg_out.at<cv::Vec3b>(i, j)[2] = 255;
					}
					else {
						seg_out.at<cv::Vec3b>(i, j)[0] = 255;
						seg_out.at<cv::Vec3b>(i, j)[1] = 0;
						seg_out.at<cv::Vec3b>(i, j)[2] = 0;
					}
				}
			}
			cv::imwrite(path + "/enc_net/" + images[index], seg_out);
		}
		return 0;
	}
...
void eval_seg_models(std::string images_gt_path, std::string images_seg_path, std::string results_txt) {
	std::vector<std::string> images = get_all_files_names_within_folder(images_gt_path);
	std::ofstream out_param(results_txt, std::ios::app);
	out_param << "facade_id";
	out_param << ",";
	out_param << "pAccuracy";
	...
	double avg_blob = 0;
	for (int index = 0; index < images.size(); index++) {
		cv::Mat seg_img = cv::imread(images_seg_path + "/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		//eval
		cv::Mat gt_img = cv::imread(images_gt_path + "/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		int gt_blobs = blobs(gt_img);
		int seg_blobs = blobs(seg_img);
		//std::cout << "gt_blobs is " << gt_blobs << std::endl;
		//std::cout << "seg_blobs is " << seg_blobs << std::endl;
		double blobs_score = 1 - 1.0 * abs(seg_blobs - gt_blobs) / gt_blobs;
		std::vector<double> evaluations = eval_accuracy(seg_img, gt_img);
		out_param << images[index];
		out_param << ",";
		out_param << evaluations[0];
		...
		out_param << evaluations[2];
		out_param << ",";
		out_param << blobs_score;
		out_param << "\n";
		avg_accuracy += evaluations[0];
		avg_precision += evaluations[1];
		avg_recall += evaluations[2];
		avg_blob += blobs_score;
	}
	out_param << "Average_Score";
	out_param << ",";
	out_param << avg_accuracy / images.size();
	...
	out_param << avg_recall / images.size();
	out_param << ",";
	out_param << avg_blob / images.size();
	out_param << "\n";
}
...
void test_ensemble_models(std::string images_path, std::string output_path, std::string model_imgs_path, std::string results_txt) {
	...
	for (int index = 0; index < images.size(); index++) {
		cv::Mat gt_img = cv::imread(images_path + "/B/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		cv::Mat seg_img_v1 = cv::imread(model_imgs_path + "/pix2pix_256_D3/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		cv::Mat seg_img_v2 = cv::imread(model_imgs_path + "/pix2pix_128_D3/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		cv::Mat seg_img_v3 = cv::imread(model_imgs_path + "/pix2pix_96_D3/" + images[index], CV_LOAD_IMAGE_UNCHANGED);
		cv::Mat seg_img(gt_img.size(), CV_8UC3);
		std::vector<std::vector<int>> color_mark;
		color_mark.resize(gt_img.size().height);
		for (int i = 0; i < color_mark.size(); i++) {
			color_mark[i].resize(gt_img.size().width);
			for (int j = 0; j < color_mark[i].size(); j++) {
				color_mark[i][j] = 0;
			}
		}
		// correct the color
		for (int h = 0; h < gt_img.size().height; h++) {
			for (int w = 0; w < gt_img.size().width; w++) {
				if (seg_img_v1.at<cv::Vec3b>(h, w)[0] == 255)
					color_mark[h][w] += 0;
				else
					color_mark[h][w] += 1;
				if (seg_img_v2.at<cv::Vec3b>(h, w)[0] == 255)
					color_mark[h][w] += 0;
				else
					color_mark[h][w] += 1;
				if (seg_img_v3.at<cv::Vec3b>(h, w)[0] == 255)
					color_mark[h][w] += 0;
				else
					color_mark[h][w] += 1;
			}
		}
		int num_majority = ceil(0.5 * 3);
		for (int i = 0; i < color_mark.size(); i++) {
			for (int j = 0; j < color_mark[i].size(); j++) {
				if (color_mark[i][j] >= num_majority) {
					seg_img.at<cv::Vec3b>(i, j)[0] = 0;
...
void test_seg2grammars(ModelInfo& mi, std::string image_path, std::string output_path) {
	...
	for (int i = 0; i < images.size(); i++) {
		...
		std::vector<double> trans_opt;
		if (predictions.size() == 5 && bOpt) {
			opt_without_doors(seg_rgb, predictions_opt, predictions, 1/* mi.opt_step*/, mi.opt_range);
			//opt_without_doors(seg_rgb, predictions_opt, trans_opt, predictions);
		}
		if (predictions.size() == 8 && bOpt) {
			opt_with_doors(seg_rgb, predictions_opt, predictions, mi.opt_step, mi.opt_range);
		...
		for (int v2 = -2; v2 <= 2; v2++) {
			for (int v3 = v3_min; v3 <= v3_max; v3++) {
				for (int v4 = v4_min; v4 <= v4_max; v4++) {
					for (int m_t = 0; m_t <= m_step; m_t++) {
						for (int m_b = 0; m_b <= m_step; m_b++) {
							for (int m_l = 0; m_l <= m_step; m_l++) {
								for (int m_r = 0; m_r <= m_step; m_r++) {
...
void findPatches(std::string image_name, std::string output_path, int step, ModelInfo& mi) {
	...
	for (int i = 0; i < w_count; i++) {
		for (int j = 0; j < h_count; j++) {
			...
			for (int width = step; width <= 3 * step; width += 5) {
				for (int height = step; height <= 3 * step; height += 5) {
	...
	int best_class = -1;
	for (int index = 0; index < rectangles.size(); index++) {
		std::string img_name = output_path + "/chip_" + std::to_string(index) + ".png";
		...
		std::cout << img_name <<", " << tmp << std::endl;
		if (tmp < best_score) {
			best_score = tmp;
			best_class = index;
		}
	}
	std::cout << "best chip is " << best_class << std::endl;
	cv::Mat best_img = cv::imread(output_path + "/chip_" + std::to_string(best_class) + ".png", CV_LOAD_IMAGE_UNCHANGED);
	cv::imwrite("../data/best_patch.png", best_img);
}
...
void img_convert(std::string images_path) {
	...
	for (int index = 0; index < images.size(); index++) {
		...
		int borderType = cv::BORDER_CONSTANT;
		cv::Mat aligned_img_padding;
		cv::copyMakeBorder(out_img, aligned_img_padding, padding_size, padding_size, padding_size, padding_size, borderType, cv::Scalar(255, 255, 255));
		// bbox and color
		std::vector<std::vector<cv::Point> > contours;
		std::vector<cv::Vec4i> hierarchy;
		cv::findContours(aligned_img_padding, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_NONE, cv::Point(0, 0));
		...
		cv::Mat dnn_img = cv::Mat(aligned_img_padding.size(), CV_8UC3, cv::Scalar(255, 0, 0));
		for (int i = 0; i < contours.size(); i++)
		{
			if (hierarchy[i][3] != 0) continue;
			if (area_contours[i] < 15)
				continue;
			cv::rectangle(dnn_img, cv::Point(boundRect[i].tl().x + 1, boundRect[i].tl().y + 1), cv::Point(boundRect[i].br().x - 1, boundRect[i].br().y - 1), cv::Scalar(0, 0, 255), -1);
		}
		dnn_img = dnn_img(cv::Rect(padding_size, padding_size, src_img.size().width, src_img.size().height));
		//cv::cvtColor(dnn_img, dnn_img, CV_BGR2GRAY);
		cv::imwrite("../data/pix2pix_sub_out/" + images[index], dnn_img);
	}
}
...
bool chipping(FacadeInfo& fi, ModelInfo& mi, ChipInfo &chip, bool bMultipleChips, bool bDebug, std::string img_filename) {
	...
	if(!bground)
		...
		cropped_chips = crop_chip_multi_scale(src_facade.clone(), type, facadeSize, targetSize);
	else
...
std::vector<ChipInfo> crop_chip_multi_scale(cv::Mat src_facade, int type, std::vector<double> facadeSize, std::vector<double> targetSize) {
	std::vector<ChipInfo> cropped_chips;
	cv::Mat src_img = src_facade.clone();
	int step = src_img.size().width * targetSize[0] / facadeSize[0];
	std::vector<cv::Rect> rectangles;
	int w_count = src_img.size().width / step;
	int h_count = src_img.size().height / step;
	int start_x = 0.5 * (src_img.size().width - w_count * step);
	...
	for (int i = 0; i < w_count; i++) {
		for (int j = 0; j < h_count; j++) {
			cv::Point center(start_x + i * step + 0.5 * step, start_y + j * step + 0.5 * step);
			for (int width = step; width <= 4 * step; width += 0.5 * step) {
				for (int height = step; height <= 4 * step; height += 0.5 * step) {
					cv::Point l1(center.x - 0.5 * width, center.y - 0.5 * height);
					cv::Point r1(center.x + 0.5 * width, center.y + 0.5 * height);
					if (l1.x < 0 || l1.y < 0)
						continue;
	...
	{
		ChipInfo chip;
		chip.src_image = src_img(rectangles[index]);
		chip.x = rectangles[index].x;
		chip.y = rectangles[index].y;
		chip.width = rectangles[index].width;
		chip.height = rectangles[index].height;
